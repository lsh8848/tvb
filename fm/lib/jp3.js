...
// homeUrl: '/api/slide/list?pos_id=88', //网站的首页链接,用于分类获取和推荐获取
	...
	filter_url: 'area={{fl.area or "0"}}&sort={{fl.sort or "hot"}}&year={{fl.year or "0"}}&category_id={{fl.cateId}}',
	filter_def: {
		0:{cateId:'0'},
		1:{cateId:'1'},
		2:{cateId:'2'},
	...
	推荐: $js.toString(() => {
        ...
        html.forEach(it => {
        let title = it.title;
        let img = 'https://vbhakzil.top' + it.path;
            d.push({
                title: title,
                img: img,
	...
	一级: $js.toString(() => {
        ...
        html.forEach(it => {
            d.push({
                title: it.title,
                img: 'https://vbhakzil.top' + it.path,
                desc: (it.mask || it.playlist.title) + ' ⭐' + it.score,
                url: it.id
            })
        });
	...
	搜索: $js.toString(() => {
        ...
        html.forEach(it => {
            d.push({
                title: it.title,
                img: 'https://vbhakzil.top' + it.thumbnail,
